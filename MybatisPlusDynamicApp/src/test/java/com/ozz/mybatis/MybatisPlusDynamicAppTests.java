...
import com.ozz.mybatis.service.MybatisPlusDynamicService;
...
@SpringBootTest
class MybatisPlusDynamicAppTests extends MyBaseTest {
    @Autowired
    MyService myService;
    @Autowired
    MybatisPlusDynamicService mybatisPlusDynamicService;
    @Autowired
    ...
    void contextLoads() {
        ...
        List<String> list = mybatisPlusDynamicService.selectDynamicDb(1L);
        println(list);
        Assert.isTrue(list.size() == 1 && "n21".equals(list.get(0)), StrUtil.EMPTY);

        list = mybatisPlusDynamicService.selectDynamicDb2(1L);
        println(list);
        Assert.isTrue(list.size() == 1 && "n21".equals(list.get(0)), StrUtil.EMPTY);
